...
删除注册表：
运行·regedit·，找到·HKEY_CLASSES_ROOT\\Directory\\Background\\shellex\\ContextMenuHandlers·，删掉·igfx·开头的
...
!!
1、在设置中将起始页改成其他网站
2、第一种设置了没用的话在网址栏输入·chrome://version·可查看版本信息
    在命令行的最后可以看到被篡改的网址，先在浏览器的快捷方式上右键属性
    在**目标**上面的最后看看有没恶意的网址，有就删除并保存，没有就点击**打开文件所在的位置**
    将·chrome·这个 exe 文件改成其他名称再打开
    改成其他名称之后会发现浏览器的图标没了，可以先发送到桌面快捷方式，在快捷方式上右键属性
    点击**更改图标>浏览>选择文件所在位置的路径>进入浏览器版本号的文件夹>选择 chrome.dll**
    然后选择想要的图标就行了
!!
...
可以使用 2 种方法
!!
进入**我的电脑右键-属性-高级系统设置-启动和故障恢复-设置**，将系统启动中的两个勾去掉
·Win + R·运行·msconfig·选择引导，将不需要的系统删除，或将超时时间设为 0
!!
...
可以在[官网查看](https://www.microsoft.com/zh-cn/windows/compare)，家庭版是没有组策略的，但其实不是没有，而是禁用了
新建一个记事本，将一下代码复制进去保存，再将后缀改成·.bat·，然后右键以管理员身份运行
...
·Ctrl + Z·后退，·Ctrl + Y·前进
按·Alt·会出现快捷方式，所以按·Alt+对应字母·就是快捷键，·Alt+数字·为自定义快捷键
...
#打印机
这里以需要一台电脑来共享的打印机为例
有的用 WiFi 就能连接的打印机直接在官网下载驱动连接就行了，只要处于同一网络
##连接的打印机脱机
...
###使用 GPT 分区后无法重装
老式的电脑一般都不支持 GPT，所以需要改回 MBR
若已出现 GPT 改 MBR 时出现分区表错误，先删除“主分区(0)”，然后建立新分区保存，再快速分区成想要的分区

###DELL 建立并使用了未知的引导模式导致开机不管任何操作都出现下划线
引导模式规则是保存在主板上的，启动时读取文件是读取硬盘上的
所以拔掉硬盘再开机就会提示无可用的文件，提示按 F2 再次进入 BIOS 删除错误的引导模式即可
...
#VS2019 快捷键同步 VSCode
工具 - 选项 - 环境 - 键盘 - 显示命令包含 - 输入：
!!
Ctrl + E：编辑.插入下一个匹配的脱字号
    移除冲突：编辑.复制（前 3 个），编辑.切换自动换行
Ctrl + D：编辑.删除行（选择第 2 个）
Ctrl + /：编辑.注释选定内容
Ctrl + Shift + /：编辑.取消注释选定内容
    移除冲突：编辑.切换块注释
Ctrl + W：窗口.关闭文档窗口
    移除冲突：编辑.选择当前字
取消波浪线提示：工具 - 选项 - 文本编辑器 - HTML(Web 窗体) - 验证 - 选项 - 取消勾选显示错误
!!
如果想知道某个快捷键当前使用的功能，点击【按快捷键】，按下快捷键，下方会有【快捷键的当前使用对象】
