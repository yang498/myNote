...
!!
parent > child：子元素，所有子元素·parent > ·
ancestor descendant：后代元素，所有后代元素·ancestor *·
prev + next：下一个指定元素，所有紧邻元素·prev +·
prev ~ siblings：后面所有指定的元素，后面所有的元素·prev ~·
!!

##属性
单个属性值的引号是可选的，即·[attr=value]·和·[attr="value"]·相等
多个属性值时就必须用引号了，比如·[attr=val1 val2]·会报错，需加上引号·[attr="val1 val2"]·
而且多个属性值的顺序也是不能变的，即匹配的是整个字符串
比如要获取·class="demo active"·需写成·$('[class="demo active"]')·，写成·$('[class="active demo"]')·获取不了，需要使用通配符
!!
[attr]¿：属性，比如·$('[class]')·表示选择含有·class·属性的元素
[attr="value"]¿：属性只等于该值，比如·$('[class="demo"]')·表示选择含有·class·属性且值只等于·value·的元素
[attr!¿="value"]¿：属性不等于该值，比如·$('[class!="demo"]')·表示选择含有·class·属性且值不等于·value·的元素
