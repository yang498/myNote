...
上面的代码中·p·的状态由·p1 p2 p3·决定，和数组的·every()·类似
...
用法同·all·，·p·的状态取决于·p1 p2 p3·中第一个改变的状态，第一个改变的返回值会传递给·p·，和数组的·some()·类似
...
用法同·all·，不管·p1 p2 p3·的状态，都触发完毕·p·就会触发·then·，·p·的参数为所有结果的返回值组成的数组
该数组的格式为固定格式的对象数组，形如：
··js
[
...
用法同·all·，·p1 p2 p3·只要有一个成功就会触发·then·，如果都失败才会触发·catch·，和·all·相反
