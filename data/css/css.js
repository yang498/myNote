...
<shape>：裁剪形状
	...
	ellipse(<rx rx> at <position>)：椭圆
		<rx rx>：分别表示 x 轴和 y 轴的半径，若填百分比将分别参考元素宽高的长度，也支持·closest-side|farthest-side·
		<position>[center]：圆心的位置，参考·background-position·
	polygon(<fill-rule>, <x y>, <x y>*))：多边形
		<fill-rule>[nonzero]：填充规则
			nonzero：叠加
			evenodd：相交
		<x y>：点的位置，即三个以上的点可连接为多边形，两个点及以下无效果，基于元素左上角为起点，支持长度和百分比
...
will-change[auto]：
	...
	<custom-ident>：自定义需要提升性能属性，例如·animation·、·transform·，可以书写多个值以逗号隔开
!!
注意：
!!
此属性最好是在需要提升性能的地方使用，而不要过度使用，否则可能会导致页面响应缓慢或者大量的内存占用
建议如果可以，在用户需要交互的时候再使用此属性，例如鼠标 hover 的时候使用，鼠标移出或动画结束时删除或改成·auto·
!!
...
##counter-reset
计数器命名：·counter-reset: <counter-name> <integer>·，可以命名多个计数器以空格隔开，通过·counter()·获取计数器的结果
!!
