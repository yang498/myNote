 ...
 * 		换肤：把相关颜色抽离出来，点击出现弹窗，可选择预定颜色和自定义调配，有滑竿和手动输入rgb和16进制颜色（3个tab选项：纯色、渐变色、图片），把选择的颜色保存在localstorage，所涉及的颜色：header和aside的背景色，h1和h2的颜色
...
const asideActive2 = () => { for(let i = $asideH2.length - 1; i >= 0; i--) if ($asideH2.eq(i).offset().top < $(window).scrollTop() + 100) return i }	// 获取当前是哪个h2
